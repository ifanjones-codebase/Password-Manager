...
class Installer(tk.Tk):
   """
   Installs missing packages without looking as scary as a cmd popup
   """
   def __init__(self, package:str):
      tk.Tk.__init__(self, package)
      self.wm_title("Installer")
      self.geometry("300x100")
      self.resizable(False,False)
      
      self.draw_ui(package)
      self.install_package(package)
   
   def draw_ui(self, package:str)->None:
      """
      draws the ui for the popup
      :param package: the package missing
      :return: None
      """
      tk.Label(
         self,
         text="Installing missing packages"
      ).pack()
      
      tk.Label(
         self,
         text=f"there is a missing python package '{package}'"
      ).pack()
      ...
      if package == "customtkinter":
         ...
         text = "needed for some button functionality"
         
      elif text == "pycryptodome":
         text = "needed for password encryption handling"
      ...
      tk.Label(
         self,
         text=f"installing now"
      ).pack()
   
   def install_package(self, package: str) -> None:
      """
      Installs a package if it's not present
      ...
      self.destroy() # closes the application so the password manager can run
...
def main()->None:
   """
   main function and a 'start' point to the program, ignore the code outside the function lmao
   :return: None
   """
